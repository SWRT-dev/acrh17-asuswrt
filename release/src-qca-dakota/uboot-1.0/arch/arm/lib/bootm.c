...
#if defined(ASUS_PRODUCT)
static void asus_boot_args(int offset)
{
	char *p;
	char commandline_buf[256];
	char *commandline = getenv("bootargs");

	sprintf(commandline_buf, "root_rfs=0x%08x ", offset);

	if (commandline) {
		/* eat leading white space */
		for (p = commandline; *p == ' '; p++);

		strcat(commandline_buf, p);
		strcat(commandline_buf, " ");
	}

	p = commandline_buf;

	strcat(p, "flash_type="CONFIG_FLASH_TYPE" ");
#if 0
	if (!strstr(p, "console="))
		strcat(p, "console=ttyHSL1,115200n8 ");
	if (!strstr(p, "rootfstype="))
		strcat(p, "rootfstype=squashfs ");
#endif

#if defined(ASUS_PRODUCT)
	if (offset) { // ASUS firmware
		if (strcmp(CONFIG_FLASH_TYPE,"nor")==0) {
			strcat(p, ROOT_MTD_DEV);
			strcat(p, " ");
		} else {
			//strcat(p, "ubi.mtd=UBI_DEV root=mtd:ubi_rootfs ");
			strcat(p, "ubi.mtd=UBI_DEV ");
		}
	}
#endif
...
static void setup_commandline_tag(bd_t *bd, char *commandline)
{
...
#if defined(ASUS_PRODUCT) // 40xx doesn't use CMDLINE_TAG
	...
	if (images.root_id < 0 || images.root_id > 2)
		images.root_id = 0;

	switch (images.root_id) {
	case 2:	/* RAM disk */
...
#if defined(ASUS_PRODUCT)
	if (images->fit_rfs_offset == 0) { // if there is no squash rootfs image offset, take it as original ramdisk
#endif
