...
#if defined(ASUS_PRODUCT)
	ulong		fit_rfs_offset;	/* init ramdisk load address */
	ulong		fit_total_len;
#endif
...
#if defined(ASUS_PRODUCT)
	ulong		img_flash_addr;
	ulong		lastAddr;	/* Latest loaded image address in RAM. */
	ulong		lastLen;
	int		root_id;	/* 0: linux, 1: linux2, 2: ramdisk, otherwise: linux */
	int		rescue_image;	/* non-zero: rescue image, don't handle RAM disk. */
#endif
} bootm_headers_t;
