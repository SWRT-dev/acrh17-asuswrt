...
static ssize_t lfp_ctrl(struct file *file, const char __user *input, size_t length, loff_t *ofs)
{
	char s[10];

	if ((length > 0) && (length < 10)) {
		if (copy_from_user(s, input, length))
			return -EFAULT;
		s[length] = 0;
...
static int lfp_read_proc(struct seq_file *m, void *v)
{
...
static ssize_t lfp_port_write_proc(struct file *file, const char __user *input, size_t length, loff_t *ofs)
{
...
static int lfp_port_read_proc(struct seq_file *m, void *v)
{
	int i, c = 0;
	struct lfp_port_s *p;
...
static int lfp_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, lfp_read_proc, NULL);
}

static int lfp_port_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, lfp_port_read_proc, NULL);
}

static const struct file_operations lfp_ctl_ops = {
	.owner   = THIS_MODULE,
	.open    = lfp_open_proc,
	.read    = seq_read,
	.write	 = lfp_ctrl,
...
static const struct file_operations lfp_port_ops = {
	.owner   = THIS_MODULE,
	.open    = lfp_port_open_proc,
	.read    = seq_read,
	.write	 = lfp_port_write_proc,
	.llseek  = seq_lseek,
	.release = seq_release_net,
};
...
static int __init init(void)
{
	...
	p = proc_create("lfpctrl", 0600, init_net.proc_net, &lfp_ctl_ops);
	if (!p)
		goto err1;

	p = proc_create("lfp_ports_ctrl", 0600, init_net.proc_net, &lfp_port_ops);
	if (!p)
		goto err2;
#endif
