...
static uint16_t find_appropriate_port6(struct net *net, const u16 zone, const uint16_t original_port, const union nf_inet_addr *ext_ip, const struct nf_nat_range *range) {
  ...
  if ((range->flags & IP_NAT_RANGE_PROTO_RANDOM)
    || (range->flags & IP_NAT_RANGE_PROTO_RANDOM_FULLY)) {
#endif
...
#else
  j = jhash2((u32 *)src, 4, range->flags & IP_NAT_RANGE_PERSISTENT ? 0 : dst->all[3] ^ zone);
#endif
...
#else
    if (!(range->flags & IP_NAT_RANGE_PERSISTENT))
#endif
...
#else
  newrange.flags       = range->flags | IP_NAT_RANGE_MAP_IPS;
  ...
  if (xt_hooknum(par) == NF_INET_PRE_ROUTING) {
...
#else
    if(range->flags & NF_NAT_RANGE_MAP_IPS) {
      newrange.min_ip = range->min_ip;
      newrange.max_ip = range->max_ip;
    } else {
      if (unlikely(ipv6_dev_get_saddr(nf_ct_net(ct), xt_out(par), &ipv6_hdr(skb)->daddr, 0, (struct in6_addr*)&newrange.min_ip) < 0))
        return NF_DROP;
      newrange.max_ip = newrange.min_ip;
#endif
    ...
    if (protonum == IPPROTO_UDP) {
...
#else
      if (!nf_inet_addr_cmp(&newrange.min_ip, &newrange.max_ip))
        src_mapping = get_mapping6_by_int_src_inrange(ip, original_port, &newrange.min_ip, &newrange.max_ip);
      else
        src_mapping = get_mapping6_by_int_src(ip, original_port, &newrange.min_ip);
#endif
      if (src_mapping != NULL && check_mapping6(src_mapping, net, zone)) {
        ...
        newrange.max = newrange.min;
        if (!nf_inet_addr_cmp(&newrange.min_ip, &newrange.max_ip)) {
          newrange.min_ip = src_mapping->addr;
          newrange.max_ip = newrange.min_ip;
        }
#endif
      } else {
...
#else
        if (!nf_inet_addr_cmp(&newrange.min_ip, &newrange.max_ip)) {
          find_leastused_ip6(zone, range, ip, &(ct_tuple_origin->dst).u3, &newrange.min_ip);
          newrange.max_ip = newrange.min_ip;
        }
        want_port = find_appropriate_port6(net, zone, original_port, &newrange.min_ip, range);
        newrange.flags = IP_NAT_RANGE_MAP_IPS | IP_NAT_RANGE_PROTO_SPECIFIED;
...
#else
static uint16_t find_appropriate_port(struct net *net, const u16 zone, const uint16_t original_port, const __be32 ext_ip, const struct nf_nat_range *range) {
#endif
  uint16_t min, start, selected, range_size, i;
...
#else
      newrange.min_ip = get_device_ip(skb->dev);
      if (unlikely(!newrange.min_ip))
        return NF_DROP;
      newrange.max_ip = newrange.min_ip;
#endif
...
#else
      if (newrange.min_ip != newrange.max_ip)
        src_mapping = get_mapping_by_int_src_inrange(ip, original_port, newrange.min_ip, newrange.max_ip);
      else
        src_mapping = get_mapping_by_int_src(ip, original_port, newrange.min_ip);
#endif
...
#else
        if (newrange.min_ip != newrange.max_ip) {
          newrange.min_ip = find_leastused_ip(zone, range, ip, (ct_tuple_origin->dst).u3.ip);
          newrange.max_ip = newrange.min_ip;
        }
        want_port = find_appropriate_port(net, zone, original_port, newrange.min_addr, range);
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(3, 10, 0)
        newrange.flags = NF_NAT_RANGE_MAP_IPS | NF_NAT_RANGE_PROTO_SPECIFIED;
        newrange.min_proto.udp.port = cpu_to_be16(want_port);
