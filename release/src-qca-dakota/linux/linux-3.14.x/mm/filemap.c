...
unsigned long pagecache_limit = 0;
...
{
	unsigned long nr_file_pages, nr_file_mapped, current_pagecache;
	int nr_pages = 0;

	nr_file_pages = global_page_state(NR_FILE_PAGES);
	nr_file_mapped = global_page_state(NR_FILE_MAPPED);
	if ( nr_file_pages && nr_file_mapped )
		current_pagecache = nr_file_pages - nr_file_mapped;
	else
		return 0;
