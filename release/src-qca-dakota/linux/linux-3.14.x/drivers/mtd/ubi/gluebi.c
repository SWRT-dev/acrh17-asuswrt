...
static const char *factory_vol_list[] = { "Factory", "Factory2" };
static struct mtd_info *factory_mtd_ptr[] = { NULL, NULL };
static int fac_mtd_resolve = 0;

static struct mtd_info *get_fac_mtd_nm(const char *name)
{
	int i;
	int err = -ENODEV;
	if (!fac_mtd_resolve) {
		...
		if (!mutex_trylock(&mtd_table_mutex))
			printk("[%s] WARNING!!! in mtd_table_mutex lock!!\n",__func__);
		mutex_unlock(&mtd_table_mutex);

		for (i = 0; i < ARRAY_SIZE(factory_vol_list); ++i) {
			mtd = get_mtd_device_nm(factory_vol_list[i]);
			if (!IS_ERR(mtd)) {
				factory_mtd_ptr[i] = mtd;
				put_mtd_device(mtd);
			}
		}
		fac_mtd_resolve = 1;
	...
	for (i = 0; i < ARRAY_SIZE(factory_vol_list); ++i) {
		if (!strcmp(name, factory_vol_list[i])) {
			if (factory_mtd_ptr[i] == NULL) {
				return ERR_PTR(err);
			}
			err = __get_mtd_device(factory_mtd_ptr[i]);
			if (err)
				return ERR_PTR(err);
			return factory_mtd_ptr[i];
		}
	}
	return ERR_PTR(err);
}
...
static int is_factory_mtd_device(struct mtd_info *mtd)
{
	...
	for (i = 0; !ret && i < ARRAY_SIZE(factory_vol_list); ++i) {
		if (!strcmp(mtd->name, factory_vol_list[i]))
			ret = 1;
	}
...
static void __list_all_eeprom_set_status(void)
{
	...
	for (j = 0; j < ARRAY_SIZE(factory_vol_list); ++j) {
		mtd = get_fac_mtd_nm(factory_vol_list[j]);
		if (IS_ERR(mtd)) {
			debug("%s: Can't get mtd_info of %s\n", __func__, factory_vol_list[j]);
			continue;
		}
...
static int __sync_eeprom_sets(unsigned char *buf)
{
	...
	for (j = 0; j < ARRAY_SIZE(factory_vol_list); ++j) {
		...
		c = div_u64_rem(mtd->size, LEB_SIZE, &rem);
		for (i = 0; i < c; ++i) {
			if (i == act_set->id && !strcmp(act_set->mtd->name, factory_vol_list[j]))
				continue;
